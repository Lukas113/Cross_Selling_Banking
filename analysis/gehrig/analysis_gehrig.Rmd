...
Jedoch ist in der Datenbank ein dritter 'type' "VYBER" (n=16666) vorhanden. Dieser wird für eine vereinfachte Analyse auf "VYDAJ" umgewandelt, was bedenkenlos gemacht werden kann, da "VYBER" in 'type' nicht definiert ist.
```{r echo = TRUE}
...
Veränderung einiger Data-Frames, um:
- Zweideutigkeiten von Variabelbenennungen bei Joins zu vermeiden
- Koorektur einiger Werte für eine vereinfachte Datenanalyse
```{r echo = TRUE}
...
Hier wird Bewiesen, dass jeder 'account' nie mehr als eine 'loan' hat.
```{r echo = TRUE}
...
Hier ist die Verteilung des Vermögens aller Kunden am Stichtag ersichtlich. Eine Minderheit der Accounts hat einen negativen Saldo. Die grösste Häufung an Accounts befindet sich etwa bei 25'000 Kronen. Die meisten Accounts haben zwischen 10'000 und 100'000 Kronen Vermögen auf ihrem Konto.
```{r echo = TRUE}
...
Hier ist die jährliche Vermögensvertielung der Bank ersichtlich. Es ist zu erkennen, dass die Bank über den ganzen Zeitraum eine ähnliche Vermögensverteilung aufweist.
```{r echo = TRUE}
...
#### Card Ownership
Hier ist ersichtlich, dass nur 'OWNER' eines Accounts eine Kreditkarte besitzen. Dies ist so nicht aus der Beschreibung her ersichtlich und lässt uns annehmen, dass dies eine Business-Rule ist, die wir beim Cross-Selling beachten müssen.
```{r echo = TRUE}
...
#### Account Vermögensveränderungen
...
#### Genauigkeitsüberprüfung der Einkommensschätzung
```{r echo = TRUE}
...
```
Die Genauigkeit des geschätzten Vermögens und der Geldflüsse ist ziemlich gut. Die meisten Abweichungen liegen zwischen den Werten 5.4 und 17.0. Die Maximaldifferenz liegt bei 1200.3. Diese ist dadurch erklärbar, da es sich bei den abgefragten Vermögensendbeständen (balance) auch nur um Schätzwerte handelt, da die Transaktionen nicht sauber sortiert werden konnten.


#### Einkommen und Vermögen verglichen mit Kreditkarten
Total Flow of Money until the Card is Issued:
- Es ist interessant zu wissen, falls jemand ein interessanter Kunde für eine Kreditkarte ist, für welche Karte (typ) dieser am meisten geeignet ist. Dazu sind Analysen auf Unterschiede der Vermögen und Vermögensveränderungen zwischen den einzelnen Kartentypen interessant.
```{r echo = TRUE, warning = FALSE, fig.width = 10}
plot_balance_change(trans_altered, account_altered, disp_altered, card_altered)
```
Es ist ersichtlich, dass Kunden mit einer höher gestuften Karte zum Zeitpunkt des Erwerbes der Karte tendenziell ein höheres Einkommen sowie höhere Vermögenszu- und Abflüsse hatten.
...
Uns ist aufgefallen, dass ein kleiner Teil der Orders nicht in "trans" enthalten sind. Es ist ebenfalls ersichtlich, dass die Orders 'LEASING' nicht eine ausgeführte Transaktion besitzt, obwohl die Zielbank- und Account in 'orders' definiert sind. Dies ist wahrscheinlich ein Fehler in der Datenbank.
