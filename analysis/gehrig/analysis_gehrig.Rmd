...
```{r echo = FALSE}
account_path <- '../objects/account.rda'
card_path <- '../objects/card.rda'
client_path <- '../objects/client.rda'
disp_path <- '../objects/disp.rda'
district_path <- '../objects/district.rda'
loan_path <- '../objects/loan.rda'
orders_path <- '../objects/orders.rda'
trans_path <- '../objects/trans.rda'
paths <- c(account_path, card_path, client_path, disp_path, district_path, loan_path, orders_path, trans_path)
...
for (path in paths){
  if (file.exists(path)){
    ...
    table_name <- unlist(strsplit(unlist(strsplit(path, '/'))[3], '.rda'))
