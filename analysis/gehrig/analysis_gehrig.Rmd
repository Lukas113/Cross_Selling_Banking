...
customer_balance_change_estimation <- function(trans_frame, without_initial_balance = FALSE, min_date = lowest_date, max_date = highest_date){
  cust_bal_ch <- trans_frame
  
  if (without_initial_balance){#filter opening balance if one is interested just in the estimated income
  ...
  cust_bal_ch <- cust_bal_ch %>%
    ...
    dplyr::summarise(balance_change = sum(amount))
...
customer_balance_change_estimation(trans_frame = trans_corrected, without_initial_balance = TRUE)
```


```{r}
t <- trans %>%
  dplyr::arrange(account_id, date, desc(trans_id)) %>%
  dplyr::group_by(account_id) %>%
  dplyr::mutate(balance_diff = (balance - dplyr::lag(balance))) %>%
  dplyr::filter(round2(abs(balance_diff), 0) == amount) %>% #ensures to just consider 100% correct calculated balance_diff
  dplyr::filter(type == 'PRIJEM') %>%
