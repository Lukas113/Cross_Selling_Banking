...
```{sql connection=con, output.var = "accounts_loans"}
...
```{sql connection=con, output.var = "card_types_loans"}
...
```{sql connection=con, output.var = "loans_demographic"}
 SELECT 
...
```{sql connection=con, output.var = "client_card_districts"}
...
```{sql connection=con, output.var = "disp_transactions"}
...
Das Zeitfenster wird als relativ zur Ausstellung der Karte beachtet

```{r}
check_services <- get_check_services()

accounts_have_card <- check_services %>%
  group_by()
