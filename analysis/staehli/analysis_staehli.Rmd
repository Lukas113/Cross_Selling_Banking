...
```{sql eval=FALSE, connection=con, include=FALSE, output.var="permanent_orders"}
...
list_of_open_connections <- dbListConnections(PostgreSQL())

print(list_of_open_connections)

#####################################################

test_SQL_query <- function(){
  con <- get_connection_SQL()
  
  get_data <- dbGetQuery(con, "SELECT account_id, count(type) as count_users from disp group by account_id")
  
  return(get_data)
}

test_data <- test_SQL_query()
...
db_connection_checker_killer <- function(){
  ## Check Connections, wenn zu viele offen dann zu.
  list_of_open_connections <- dbListConnections(PostgreSQL())
  
  if(length(list_of_open_connections) > 2) {
    for(con in list_of_open_connections){
      dbDisconnect(con)
    }
  }
}

db_connection_checker_killer()
