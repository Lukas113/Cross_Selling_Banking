...
```
  ...
  - Aus District 1 (Prag) werden am meisten loans bezogen
  - Höchste Outliers ebenfalls in D1
...
client_card_districts <- dbGetQuery(con,"SELECT card.type AS card_type, disp.client_id, client.district_id,
...
head(client_card_districts)

# Neue Tabelle gruppiert mit district_i und anzahl karten gesamt
ggplot(clients_with_card, aes(x = district_id, y = cards_per_district)) +
  geom_col(alpha = 1/2) +  xlim(1,77) +  scale_x_continuous(breaks = 1:77) 

#Amount of cards in district 
ggplot(client_card_districts, aes(x = district_id, color = card_type)) +
  geom_freqpoly(binwidth = 1, alpha = 3/4) + labs(title = "Amount of customers in districts" ) +
