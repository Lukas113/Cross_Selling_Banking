...
Da die Datensätze relativ klein sind, erschien es sinnvoll, die Tables als R-Objekte lokal abzuspeichern, um diese danach lokal bearbeiten zu können. Die Datenobjekte werden vom entsprechenden Ordner geladen, falls das Objekt und der Ordner vorhanden ist. Falls diese nicht vorhanden sind, werden sie neu von der Datenbank geholt und lokal ins Ordnerverzeichnis abgespeichert.
...
# Qualität und Representativität der Daten
...
In der Tabelle trans sind einige Werte in `k_symbol`, `bank`, `account` und `operation`: «NA», « », oder «0». In diesem Abschnitt werden die Vorkominisse dieser Unregelmässigkeiten untersucht, welche nicht in der Beschreibung erwähnt wurden, um die Daten besser zu verstehen und eventuelle Unstimmigkeiten aufzudecken. Folgende Kombinationen von den erwähnten Werten in den Attributen sind aufgefallen:
...
In diesem Abschnitt wird der `type` der Transaktionstabelle untersucht, da während der Vermögensanalyse grosse Unstimmigkeiten bezüglich der Vermögen aufgefallen sind.
...
- Bei Zweideutigkeiten von Variabelbenennungen wird der jeweilige Tabellenname kombiniert, um Komplikationen bei Joins zu vermeiden. Bsp: `date` der Tabelle trans wird zu `trans_date`.

- Korrektur von Fehlerhaften Werten, Bsp: `trans_type` «VYBER» wird gemäss obiger Erkentniss zu «Abfluss».
```{r echo=FALSE}
...
Während der Datenanalyse ist aufgefallen, dass nicht alle `orders` in der Tabelle trans abgebildet sind. Aus diesem Grund erfolgt eine Validierung, auf welchen Zeilen der Tabelle orders überhaupt Transaktionen ausgeführt wurden. Geprüft wurde auf `account_id` und `account_to`, da in der Tabelle trans nicht direkt die Verbindung zu der Tabelle orders gegeben ist. Zusätzlich ist bei der Tabelle orders auch kein Anfangs- und Enddatum angegeben, was eine Validierung erschwert. Wenn eine Transaktion von einem `account` zum in `orders` definierten `account_to` vorhanden ist, sowie der `amount` von `trans` und `orders` übereinstimmt, wurde die Order mit hoher Wahrscheinlichkeit ausgeführt. Somit werden hier diejenigen Transaktionen herausgefiltert, bei denen 100% nie eine Transaktion ausgeführt wurde.
...
Hier wurde die Annahme bearbeitet, dass ein gewisser Kartentyp einem spezifischen Bezirk zugeordnet werden kann. Aus optischen Gründen um die Bezirke sauber darzustellen, wurde hier anstatt mit den Bezirken, mit den Regionen gearbeitet. Aus dem Plot ist ersichtlich, wieviele Kunden aus welchen Regionen stammen. Ausserdem kann man sehen, dass die Verteilung der Karten auf die einzelnen Regionen nicht viel hergibt und es nach der Anzahl her relativ gleich verteilt ist.
...
Mit dem Barchart wird grob aufgezeigt wieviele Konten über einen Kredit verfügen und wieviele nicht.  Er zeigt, dass Potenzial an Kunden vorhanden wäre, die einen Kredit aufnehmen könnten. Der Plot identifiziert ein mögliches Geschäftsfeld der Bank zur Gewinnung potenzieller Kunden.
...
Mit diesem Plot wurde die Hypothese, dass es Signifikante Unterschiede nach Geschlecht gibt bei der Vergabe von Krediten, bearbeitet. Er wiederlegt, dass die Hypothese nicht korrekt ist. Es sind keine grossen Unterschiede vorhanden in der Anzahl, sowohl auch beim Kreditstatus. 

Beim zweiten Plot wurden Altersgruppierungen erstellt.  
Anzahl der Kunden nach Alter: 
...
Die Verteilung der Karten unterscheidet sich stark. Der `card_type` «classic» ist relativ gesehen übervertreten. Da «gold» und «junior» eine ziemlich kleine Dichte aufweisen, müssen wir bei der Wahl des Timeframes vorsichtig vorgehen, um noch genügend Kunden mit «gold» und «junior» in der Analyse miteinzubeziehen.
...
Der untere Graph ist das Alter der Kunden bei ausstellung der Kreditkarte. Der obere Graph sind alle Kunden und Alter welche keine Kreditkarte haben zum Stichtag 31.12.1998.
...
Insgesamt gibt es 514 Kunden welche Besitzer («owner») eines Accounts sind, welche keine Kreditkarte besitzen und unter 25 Jahre sind. Dies wären potentielle Kunden für die Junior-Kreditkarte.  
Die ersten 6:
```{r echo=FALSE}
...
Insgesamt gibt es 3094 Kunden welche Besitzer («owner») eines Accounts sind, welche keine Kreditkarte besitzen und über 24 Jahre sind.  
Die ersten 6:
```{r echo=FALSE}
...
## Konsolidierter Datensatz

Um den konsolidierten Datensatz zu erstellen, empfehlen wir:

- die tschechischen Begriffe zu übersetzen auf Englisch
- die Kundendaten anpassen:
  - Geburtsdatum extrahieren
  - Geschlecht extrahieren
  - Aktuelles Alter berechnen
  - Nutzung (ja/nein) von «Loans»
  - Nutzung (ja/nein) von «Permanent Orders»
  - Nutzung von «Cards»
  - Kreditkartentyp falls Karte vorhanden
- Anpassung der Konten:
  - Durchschnittliche monatliche Vermögensveränderung der letzten 12 Monate
  - Aktuelle Vermögen einfügen

## Nächste Schritte
Wir empfehlen nach Erstellung des konsolidierten Datensatzes die Erstellung eines Modelles auf Basis dieser Analyse um die Kundenselektion auszuarbeiten. Anhand der Analyse empfiehlt sich die Ausarbeitung eines Descision tree's, um die Kundenselektion auszuarbeiten.  
Das Vorgehen und die Details der weiteren Schritte muss zusammen mit den beteiligten Abteilungen ausgearbeitet werden.
